...
def Device_reader():
     ...
     ChemChk = (int(Time.time())- ChemT)/86400
     PumpChk = (int(Time.time())- PumpT)/86400
     print(ChemChk,PumpChk)
     print(tg,sn,ads.readADCSingleEnded(channel=3), 49.26*ads.readADCSingleEnded(channel=0)/2500, data.temperature, data.humidity, DewPf,int(0.2*ads.readADCSingleEnded(channel=2)-250))
     values= [tg,sn,int(abs(Current)),int(Tank),int(Airrh),int(Airt),int(DewP), int(Oventemp),int(ChemChk),int(PumpChk)]
     return(values)
#---------------------------------------------------------------------------# 
